...
**재생 중 표시** — 소리가 나고 있는 문장은 초록 배경으로 표시됩니다(`.dialogue.playing`). 예전에는 전체 재생만 이 표시를 붙여서, 문장을 클릭하거나 키로 옮겨 재생하면 **어디가 재생 중인지 화면에 아무 단서가 없었습니다.**

표시는 `toggle.js`가 `play`/`pause`/`ended` 이벤트로 붙이고 뗍니다. 클릭·키보드·전체 재생·잠금화면 어느 경로로 시작해도 같은 표시가 나오고, 나중에 만들어지는 대화문도 자동으로 포함됩니다. **`.playing`을 다른 곳에서 직접 건드리지 마세요** — `play-all.js`는 스크롤만 담당합니다.
