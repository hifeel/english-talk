...
| 시나리오 | 117개 |
| 대화 문장 | 1,108개 |
| 오디오 | mp3 1,108개 (약 22MB, 저장소에 직접 커밋) |
...
### 무음 정리 — `scripts/trim_silence.py`

**새로 만든 mp3는 커밋 전에 이 스크립트를 돌리세요.** TTS가 돌려주는 음성에는 말이 아닌 무음이 붙어 있습니다.
...
앞뒤 패딩은 문장부호와 무관하게 **모든 파일**에 붙습니다. 전체 1,108개 기준 이것만으로 약 20분이었고, 정리 후 총 재생 시간이 87.8분 → 62.0분으로 줄었습니다 (`8dbfe2d`).

```sh
python3 scripts/trim_silence.py                    # audio/ 전체
python3 scripts/trim_silence.py audio/hotel_*.mp3  # 특정 파일만
python3 scripts/trim_silence.py --dry-run          # 확인만
```

| 옵션 | 기본값 | 설명 |
|---|---|---|
| `--gap SEC` | 0.25 | 문장 내부 무음 상한 |
| `--threshold DB` | -45 | 무음으로 볼 음량 |
| `--force` | | 이미 정리된 파일도 다시 처리 |
| `--dry-run` | | 변경 없이 대상만 출력 |

**이미 정리된 파일은 건너뜁니다.** 재실행이 무해하므로 새 음성만 골라낼 필요 없이 그냥 전체를 돌려도 됩니다. mp3 재인코딩이라 `--force`를 반복하면 음질이 조금씩 깎이니, 값을 바꿔 다시 만들 때는 원본에서 다시 시작하세요.

발화가 잘리는 것을 막기 위해 앞 60ms·뒤 150ms를 남깁니다. 결과가 원본의 35% 미만으로 짧아지면 필터가 말을 삼킨 것으로 보고 **원본을 그대로 두고** 종료 코드 1로 알립니다.

`ffmpeg`가 필요합니다 (`brew install ffmpeg`). 인코딩은 원본과 같은 mp3 48kbps / 24kHz / 모노로 맞춥니다. `audio/conversations/`는 원본 보관용이라 건드리지 않습니다.
...
**오디오는 설치 때 받지 않습니다.** mp3 1,108개 22MB를 한 번에 받게 할 수 없어서, 카테고리 페이지의 **"오프라인 저장" 버튼**으로 필요한 것만 받습니다(`js/offline.js`). 저장하지 않은 카테고리의 mp3는 네트워크로 직행하므로, 오프라인에서는 화면만 뜨고 소리는 나지 않습니다.

캐시는 두 개로 나뉩니다. 셸은 `english-talk-v7`, 오디오는 `english-talk-audio-v2`입니다. **셸 버전을 올려도 오디오 캐시는 지워지지 않아야** 하므로 activate 핸들러가 두 이름을 모두 예외 처리합니다.
