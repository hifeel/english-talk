...
js/keyboard.js       대화문 페이지 단축키 (scenario.html, marks.html)
```
...
캐시는 두 개로 나뉩니다. 셸은 `english-talk-v11`, 오디오는 `english-talk-audio-v2`입니다. **셸 버전을 올려도 오디오 캐시는 지워지지 않아야** 하므로 activate 핸들러가 두 이름을 모두 예외 처리합니다.
...
**키보드 조작** — `js/keyboard.js`가 대화문 페이지에 단축키를 붙입니다. 한 문장을 반복해 들을 때 다음 문장으로 넘어가려고 매번 마우스를 잡지 않아도 됩니다.
...
전체 재생 중에는 그 실행이 순서를 소유하므로 이동을 `etPlayAllSkip()`에 넘기고, 아니면 인접 문장을 직접 재생합니다. 반복 모드가 켜져 있으면 옮겨간 문장에도 반복이 이어집니다.

입력란·버튼·`<audio>` 컨트롤에 포커스가 있을 때는 **가로채지 않습니다.** 버튼 위에서 Space는 버튼을 누르는 것이 맞고, 오디오 컨트롤 위에서 화살표는 탐색이 맞기 때문입니다. 수정키(Ctrl/Cmd/Alt)가 눌린 조합도 브라우저에 넘깁니다.
