...
**캐시하는 것** — 앱 셸(HTML·CSS·JS)과 `data/*.json` 전부. 합쳐서 약 337KB라 설치 시점에 통째로 받습니다.

**캐시하지 않는 것 — 오디오.** mp3 1,108개 30MB를 설치할 때 전부 받게 할 수는 없어서, 지금은 네트워크로 직행합니다. 즉 **오프라인에서 텍스트는 다 보이지만 소리는 나지 않습니다.** 카테고리별 선택 다운로드가 붙어야 완성되며, 그때까지는 의도된 한계입니다.

서비스워커가 mp3 요청을 아예 가로채지 않는 데는 이유가 하나 더 있습니다. 캐시는 Range 요청에 전체 파일을 200으로 돌려주기 때문에, 가로채면 **오디오 시크가 깨집니다.**
